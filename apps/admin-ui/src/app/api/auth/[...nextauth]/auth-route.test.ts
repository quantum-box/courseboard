...
describe('courseboard Auth.js route adapter', () => {
	...
	it('does not accept workers.dev callback URLs for auth redirects', () => {
		expect(
			normalizeAuthCallbackUrl(
				'https://courseboard.quantum-box.workers.dev/dashboard?from=auth',
			),
		).toBe('/')
		expect(
			normalizeAuthCallbackUrl(
				'https://pr357--courseboard.txcloud.app/dashboard?from=auth',
			),
	...
	it('normalizes POST provider sign-in before Auth.js sees the action', async () => {
		const request = new Request(
			'https://courseboard.quantum-box.workers.dev/api/auth/signin/cognito',
			{
				body: new URLSearchParams({
					callbackUrl: 'https://courseboard.quantum-box.workers.dev/dashboard',
		...
		expect(authRequest.method).toBe('POST')
		expect(new URL(authRequest.url).toString()).toBe(
			'https://courseboard.txcloud.app/api/auth/signin/cognito',
		)
		expect(await authRequest.text()).toBe(
			'callbackUrl=%2F&identity_provider=Google',
		)
	...
	it('normalizes standard auth requests to the resolved auth host', () => {
		const request = new Request(
			'https://courseboard.quantum-box.workers.dev/api/auth/signin',
			{
				headers: {
