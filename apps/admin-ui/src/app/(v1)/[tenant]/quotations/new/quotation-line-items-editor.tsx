...
export function QuotationLineItemsEditor() {
	const [rows, setRows] = useState<LineItemRow[]>(
		Array.from({ length: INITIAL_ROW_COUNT }, (_, index) => ({ id: index + 1 })),
	)
	...
	return (
		<section className='min-w-0 space-y-3'>
			...
			<div className='hidden grid-cols-[36px_minmax(220px,1fr)_82px_118px_118px_118px_36px] gap-2 border-y bg-muted/30 px-2 py-2 text-xs font-medium text-muted-foreground md:grid'>
				<div>No.</div>
				<div>品目</div>
				<div>数量</div>
				<div>単価</div>
			...
			<div className='space-y-2 md:space-y-0 md:border-b'>
				{rows.map((row, index) => (
					<div
						key={row.id}
						className='grid gap-2 border-y py-3 md:grid-cols-[36px_minmax(220px,1fr)_82px_118px_118px_118px_36px] md:items-end md:border-y-0 md:border-t md:px-2 md:py-2'
					>
						...
						<div>
							...
							<Input
								name='description'
								placeholder='サービス利用料'
								defaultValue={index === 0 ? 'サービス利用料' : undefined}
								required={index === 0}
								className='h-9'
							/>
						</div>
						<div className='grid grid-cols-2 gap-2 sm:grid-cols-4 md:contents'>
							...
							<div>
								<Label className='md:hidden'>値引</Label>
								<Input
									name='discountAmount'
									type='number'
									min='0'
							...
							<div>
								<Label className='md:hidden'>税額</Label>
								<Input
									name='itemTaxAmount'
									type='number'
									min='0'
