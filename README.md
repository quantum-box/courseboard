...
Course Board は、TACHYON Field の汎用 ERP capability と連携するオープンソースの
Cloud App 実装です。ゴルフ固有のドメインロジック、Field API adapter、migration、
テスト、UI はこの repository で管理します。

完全な実行には、互換性のある Field API、必要な tenant policy、OIDC/OAuth 設定、
MySQL 互換データベース（TiDB など）が必要です。production の database、顧客データ、
access token、client secret、署名鍵、provider credential は repository に含めず、
deployment environment から注入します。

公開 client ID や service URL は credential ではありませんが、tenant 固有の設定や
運用記録を追加するときは、公開範囲とデータの性質を確認してください。詳細は
[OSS publication boundary](docs/oss-publication.md) を参照してください。
...
運用値は deployment environment で管理してください。
...
| 用途 | platform テナント |
| -- | -- |
| 本番 | deployment-managed platform tenant |
| サンドボックス | deployment-managed sandbox tenant |
