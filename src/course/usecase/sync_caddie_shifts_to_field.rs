...
use crate::course::domain::{
    CaddieShiftGateway, CourseError, GatewayCredentials, GolfOpsGateway, ShiftRulesGateway,
    UnsyncedShift, YearMonth,
};
...
impl SyncCaddieShiftsToFieldUseCase {
    ...
    pub async fn execute(
        ...
        let hours = self
            ...
            .await?;
        let staff_by_caddie: std::collections::HashMap<&str, &str> = roster
            .caddies()
            .iter()
            .filter_map(|caddie| Some((caddie.id().as_str(), caddie.staff_id()?)))
            .collect();
            ...
            let pushes = group.iter().map(move |entry| {
                ...
                async move {
                    let link = self
                        ...
                        .await?;
                    Ok::<_, CourseError>((outcome(entry, staff_id), link))
                }
            });
            for (outcome, link) in futures::future::try_join_all(pushes).await? {
                match outcome {
...
mod tests {
    ...
    fn use_case(
        ...
        let use_case = SyncCaddieShiftsToFieldUseCase::new(
            Arc::new(FakeOps {
                roster: CaddieRoster::new(roster, Vec::new()),
            }),
            shifts.clone(),
            Arc::new(FakeRules),
