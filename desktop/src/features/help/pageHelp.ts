...
const helpByRoute: Record<string, PageHelp> = {
  golf: {
    ...
    usage: [
      {
        heading: 'まず運用タイムラインを開く',
        body: '右上の「運用タイムラインを開く」から当日ボードへ直接移動できます。予約とキャディ割当のギャップ確認から始める運用に合わせてあります。',
      },
      {
        heading: '運用機能タイルを使う',
        body: '「運用機能」の各タイル（運用タイムライン、ゴルフ予約商品、名簿、配置、勤怠、給与、予算マスタ、月次精算、キャンセル料）を押すと該当画面へ移動します。コースマップはサイドバー「コース予約」から、コース管理と予約ポリシーは設定（テナントマスタ）から開きます。',
      },
      {
        heading: '今日の運用順序に沿う',
        body: '下の「今日の運用順序」は 01 タイムライン → 02 キャディ配置 → 03 売上と請求 → 04 月次精算 です。同じ予約・キャディ・請求データを順番に引き継ぐ想定なので、途中の画面だけを飛び飛びに更新すると整合が崩れやすくなります。',
      },
      {
        heading: '01 タイムライン',
        body: '運用タイムラインで当日のティーシートとキャディレーンを見ます。未割当や衝突があれば配置へ進みます。商品・枠の見直しはゴルフ予約商品、受付ルールは設定の予約ポリシーで、マスタ整備時に行います。',
      },
      {
        heading: '02 キャディ配置',
  ...
  'golf/products': {
    ...
    usage: [
      ...
      {
        heading: '関連画面',
        body: '通貨・タイムゾーンは設定の「ゴルフ拡張設定」。コースは設定のコース管理。受付条件は設定の予約ポリシー。名簿の希望休・配置のキャディ付枠とあわせて運用します。',
      },
    ],
    data: [
      {
  ...
  'golf/policy': {
    title: '予約ポリシー',
    summary:
      'テナント導入時に整える受付ルールです（基本条件、デポジット、セルフロック、客単価判定）。日常運用のサイドバーには出さず、設定から開きます。',
    usage: [
      {
        heading: '開き方',
        body: 'アカウントメニューまたは ⌘K の「設定」→「テナントマスタ」の「予約ポリシー」から開きます。画面上部の「設定へ戻る」で設定ハブに戻れます。',
      },
      {
        heading: '設定の有無を確認する',
  ...
  'golf/timeline': {
    title: '運用タイムライン',
    summary:
      '当日の予約とキャディ割当を、タイムラインまたはタイルで確認する運用ボードです。未割当・衝突を見つけ、配置画面へつなぎます。',
    usage: [
      {
        heading: '日付とコースを切り替える',
        body: '上部の日付コントロールで日を移動し、「コース」で絞り込みます。開発モックでは 2026-07-18 にデモ用の一日分データが入っています。',
      },
      {
        heading: 'タイムラインとタイルを切り替える',
        body: 'ツールバーの「タイムライン / タイル」で表示モードを切り替えます。割当の有無を素早く見るだけならタイル、時間帯の重なりや進行を見るならタイムラインが向いています。選択はブラウザに保存されます。',
      },
      {
        heading: 'タイル表示を読む',
        body: 'コースごとに予約枠が並びます。各タイルにティー時刻・組名・人数・割当状況・担当キャディが出ます。未割当は砂色、衝突は赤枠です。',
      },
      {
        heading: 'ティーシートを読む',
        body: 'タイムライン上段はコースごとの予約レーンです。ブロックの長さは想定所要時間、色とラベルでキャディ付き／セルフ／未割当を区別します。縦の「現在」線はデモ現在時刻（09:00）です。',
      },
      {
        heading: 'キャディレーンを読む',
        body: 'タイムライン下段はキャディごとの割当バーです。同じキャディで時間が重なるバーは「衝突」（赤枠）になります。行メタにはランクとスキルが出ます。',
      },
      {
        heading: 'レーン表示とズーム',
        body: 'タイムライン時は「両方 / ティーシート / キャディ」と − / % / ＋ のズームが使えます。ピンチまたは ⌘/Ctrl + スクロールでも拡大縮小できます。',
      },
      {
        heading: '詳細と配置へ進む',
        body: 'タイルやブロックを選ぶと右の詳細に組情報・割当状況・担当キャディが出ます。未割当や衝突があるときは「配置へ」「配置を開く」から配置画面で担当を確定します。',
      },
      {
        heading: '指標の見方',
        body: '上部メトリクスは当日のティー数、割当済、未割当、衝突組数です。充足率はキャディ必須予約に対する主担当割当の割合です。',
      },
    ],
    data: [
      {
      ...
      {
        heading: '割当状況',
        body: '予約ごとに「割当済」「一部割当」「未割当」「不要（セルフ）」を示します。セルフはキャディが不要な組、未割当はキャディ付きなのに担当がまだ決まっていない組です。メトリクスの充足率は、キャディが必要な組のうち主担当が付いている割合です。',
      },
      {
        heading: '衝突',
        body: '同じキャディが、時間の重なる複数の予約に同時に割り当てられている状態です。現場では一人が二組を同時にカバーできないため、赤枠で警告し、配置画面で担当を直します。',
      },
    ],
  },
  ...
  settings: {
    title: '設定',
    summary:
      '初期設定や普段ほぼ触らない項目のハブです。テナントマスタ（コース管理・予約ポリシー）、Extension runtime、ゴルフ拡張設定（通貨・タイムゾーン）、連携メタデータをまとめています。',
    usage: [
      {
        heading: 'テナントマスタを開く',
        body: '「テナントマスタ」から「コース管理」（コース名・ホール数・スタート間隔）と「予約ポリシー」（受付条件・デポジット・セルフロック・客単価判定）を開けます。導入時に一度整えれば、日常のサイドバーには出しません。',
      },
      {
      ...
      },
      {
        ...
        body: '下の「連携メタデータ」は PMS / OTA 連携用の任意 JSON です。プレースホルダとユースケース例を参考にキーを入れ、「メタデータを保存」します。日常の受付運用では空の {} のままで構いません。予約ポリシー本体は上の「テナントマスタ」から開きます。',
      },
      {
        heading: 'その他の設定変更の場所',
        body: 'プレー区分・受付枠はサイドバーの「ゴルフ予約商品」画面です。',
      },
    ],
    data: [
      {
        heading: 'Tenant masters',
        body: 'コース管理・予約ポリシーなど、導入時マスタ。ルート例: golf/courses、golf/policy。設定ハブから遷移します。',
      },
      {
