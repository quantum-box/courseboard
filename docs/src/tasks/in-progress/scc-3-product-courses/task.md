...
> **(2026-08-23 追記)** 商品の実体である extension config の配列は、
> [ADR-0010](../../../architecture/decisions/ADR-0010-courseboard-is-not-a-field-extension.md)
> の撤退で**最後まで残る 1 系統**になる。Field の公開ストアフロントが同じ配列を
> 商品カタログとして読んでおり、Field 側に商品テーブルが無いため。
> ゴルフの key（プレー種別、ホール数、対象コース、1 組の上限人数）は Field が
> 読んでいないので先にローカル DB へ出せる。本 taskdoc が進めている
> 「対象コースを汎用の許可リストで表す」形は、**その分割の前提条件**でもある。
> 全商品が許可リストを宣言済みになれば、legacy のゴルフ用 key を config から落とせる。
