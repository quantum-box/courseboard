...
# Neither cache is repaired by leaving early: a generation can carry a rotten
# git checkout while holding no registry sources at all, which is exactly the
# shape that stopped a deploy once this script only knew about the registry.
if [ ! -d "$sources" ]; then
  echo "No Cargo registry source cache at $sources; nothing to repair there"
else
  repaired=0
...
if [ ! -d "$checkouts" ]; then
  echo "No Cargo git checkout cache at $checkouts; nothing to repair there"
  exit 0
fi
